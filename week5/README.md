...
- (5주차 추가)로컬 스토리지에 accessToken이 존재시 folder.html로 이동합니다.
- signUp 과의 공통되는 이벤트들을 모은 함수를 호출하고, 폼 제출 이벤트 발생시 로그인 api에 접근하는 함수를 호출합니다.

### signUp.js

- (5주차 추가)로컬 스토리지에 accessToken이 존재시 folder.html로 이동합니다.
- signIn 과의 공통되는 이벤트들을 모은 함수를 호출하고, 폼 제출 이벤트 발생시 입력값의 유효성을 검사하는 함수를 호출 후 회원가입 api에 접근하는 함수를 호출합니다.

## js/modules

### domElements.js

- Linkbrary 페이지의 DOM 요소에 접근하는 변수들을 모아놓은 모듈입니다.

### validator.js

- 로그인과 회원가입 페이지에서 입력값의 유효성을 검증하는 모듈입니다.

### authDOMHandler.js

- 로그인과 회원가입 페이지에서 이벤트 발생으로 인한 경고메세지와 경고성 테두리를 다루는 모듈입니다.

### authEventHandler.js

- 로그인과 회원가입 페이지에서 발생하는 공통 이벤트를 모아놓은 모듈입니다.

### authApiUtils.js

- 로그인과 회원가입 페이지에서 api에 접근이 필요한 이벤트 발생시 api에 접근 및 후처리를 담당하는 모듈입니다.
...
- 4주차 코드리뷰를 반영한 **대대적인** 리팩토링 (직관적인 함수 및 파일명, 이증검증 문제 해결)
- api를 이용한 로그인 기능 구현
- api를 이용한 이메일 중복 체크 기능 구현
- api를 이용한 회원가입 기능 구현
- api를 통해 전달받은 accessToken 로컬 스토리지 저장 기능 구현
- 로컬 스토리지의 accessToken 존재 여부에 따른 folder.html 리다이렉트 기능 구현
