...
// 에러이벤트 표시용 함수
function showErrorMsg(errorMsg, input, message) {
...
// 에러이벤트 숨김용 함수
function doneErrorMsg(errorMsg, input) {
...
function emailErrHandler() {
  if (emailInput.value === "") {
    showErrorMsg(emailErrorMsg, emailInput, "이메일 주소를 입력하세요.");
    return;
  } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailInput.value)) {
    showErrorMsg(emailErrorMsg, emailInput, "올바른 이메일 주소가 아닙니다.");
    return;
  } else if (emailInput.value === "test@codeit.com") {
    showErrorMsg(emailErrorMsg, emailInput, "이미 사용 중인 이메일입니다.");
    return;
  } else {
    doneErrorMsg(emailErrorMsg, emailInput);
  }
}
//패스워드인풋 에러핸들러
function pwErrHandler() {
  if (pwInput.value === "") {
    showErrorMsg(pwErrorMsg, pwInput, "비밀번호를 입력해주세요.");
    return;
  } else if (
    ...
    doneErrorMsg(pwErrorMsg, pwInput);
  }
}
//패스워드체크인풋 에러핸들러
function pwCkdErrHandler() {
  if (pwInput.value !== pwCheckInput.value) {
    showErrorMsg(pwCheckErrorMsg, pwCheckInput, "비밀번호가 일치하지 않아요.");
    return;
  } else {
    doneErrorMsg(pwCheckErrorMsg, pwCheckInput);
  }
}
